...
> *(පසුව ලියන ලදී — ඒ වෙලාවේ worklog එකට ලියවී තිබුණේ නෑ.)*

## ⏱️ සාරාංශය

| | |
|---|---|
| **කරපු දේ** | T7.1 ✅ T7.2 ✅ T7.6 ✅ T7.6b ✅ T7.8 ✅ · §V draft ✅ · workload diagnosis ✅ |
| **Phase තත්ත්වය** | Phase 7 **5/9** (figures 3ක් + T7.7 ඉතුරු) |
| **Tests** | **236 pass** |
| **ඊළඟ අදියර** | 🔵 T7.3, T7.4 figures → T7.7 → **Phase 8 (paper)** |

---

# ★★★ මේ session එකේ ලොකුම සොයාගැනීම

```
Framework එක steer කරන්නේ  objective එක  නෙවෙයි  —  CONSTRAINTS.
```

Weight configurations **4ම වෙනසක් නෑ**: mean fatigue **0.541 – 0.542**, throughput **90.2 – 90.3** — human weights **7 ගුණයකින්** වෙනස් වුණත්.

**ඇත්ත හේතුව:**
```
Hard constraints filter කළාට පස්සේ...
  95.8%  තීරණවලට  candidate 0 හෝ 1යි     →  කිරන්න දෙයක් නෑ
   4.2%  ඇත්ත තේරීමක් තියෙන ඒවායේ         →  හොඳම/නරකම පරතරය median 0.016
```

> ★★ **මේක වහන්න දෙයක් නෙවෙයි — මේක තමයි paper එකේ තර්කය.** අපි හැම තැනකම කියන්නේ
> *"hard constraints optimizer එකට කලින්"* කියලා. **මේක ඒකේ සෘජු මිනුම.**

**W-Profit ගැන — expplan §10 හි අනාවැකියෙන් අඩක් හරි, අඩක් වැරදි:**

| අනාවැකිය | ප්‍රතිඵලය |
|---|---|
| W-Profit → B2 ගේ throughput එකට | ✅ **හරි** — 1.8% ඇතුළත |
| W-Profit → B2 වගේම වෙනවා | ⛔ **වැරදියි** — fatigue හෝ breaches එකට **කවදාවත් බෑ** |

➜ ✅ **ලියන්න:** *"Industry 4.0 is the limit of the **objective**, not of the **framework**."*
Human weights ඔක්කොම 0 කළත් fatigue limit, competence floor, ergonomic ceiling **තාම filter කරනවා**.

---

## 📦 කොටස 1 — T7.1 + T7.2 · KPI table (`74bcd66`)

**Table II:** KPI 11 × scenario 3 × baseline 3. **Mann-Whitney U** + **Cliff's delta** + Bonferroni (α = **0.0045**).

> **ඇයි rank tests, t-test නොවේ?** runs 30ක් වැඩි නෑ · KPI කිහිපයක් bounded ·
> එකක් B3 යටතේ **හරියටම බිංදුව**. Normality test කරලා **වාර්තා** කරනවා, ඒත් ඒක මත **රඳා පවතින්නේ නෑ**.

### ⚠️ Tests මුලින්ම ලියලා — **වාර්තාකරණ දෝෂ 2ක් අල්ලගත්තා**

| # | දෝෂය | නිවැරදි කිරීම |
|---|---|---|
| 1 | % ගණනය **full precision** මගින්, ඒත් table එකේ තිබුණේ **round කරපු** අගය | පාඨකයෙක් නැවත ගණන් කළොත් ගැලපෙන්නේ නෑ (workload එකේ **පොයින්ට් 5ක්** අඩුයි). දැන් **printed අගයෙන්** |
| 2 | බිංදුවට ළං පදනමක % එකක් = **අර්ථ ශූන්‍යයි** | Gini 0.009 → 0.084 කියන්නේ **+863%**. දැන් ඒ පේළිවලට **නිරපේක්ෂ වෙනස** (`+0.076 abs`) |

---

## 📦 කොටස 2 — T7.6b · λ, μ sensitivity (`e219868`) 🚪

**ඔබ බය වුණු ප්‍රශ්නය මෙතනින් වැහුණා.** λ සහ μ එකට **0.5× · 1× · 2×** (හතර ගුණයක පරාසයක්).

```
                    0.5×      1×       2×
Fatigue වාසිය      25.4%    27.0%    33.1%     ← B3 අඩුවෙන්
Throughput වියදම    0.8%     1.8%     5.9%
Breaches            0        0        0        ← හැම තැනම
Ranking             ✅        ✅        ✅       ← කවදාවත් පෙරළෙන්නේ නෑ
```

> ★ **ඇයි මේක ඒත්තු ගන්වනවා:** රටාව **පැතලි නෙවෙයි, එකඟයි**. වෙහෙස වේගෙන් එනකොට
> framework එකේ වාසියත් වැඩි වෙනවා, ඒකට යන වියදමත් වැඩි වෙනවා — **භෞතිකව අර්ථවත්**.

> 🛡️ **Test එකෙන් ආරක්ෂා කරන දේ:** *knob එක කිසිවක් නොකරන study එකකුත් pass එකක්මයි වාර්තා කරන්නේ.*
> ඒ නිසා scaling එක ① config එකට **ඇත්තටම යනවද** ② recovery > accumulation **රැකෙනවද**
> ③ ප්‍රතිඵලය **දෘශ්‍යමානව වෙනස් වෙනවද** — **ranking බලන්න කලින්** මේ 3ම assert කරනවා.

✅ **GATE 3 carry-over එකෙන් rates දෙක වැහුණා.** *(Threshold එක Session 7 එකේ · coupling coefficients ablation එකෙන්.)*

---

## 📦 කොටස 3 — T7.8 · CP1–CP5 Ablation (`a91d8e7`)

### ★ ක්‍රමවේදය — වැදගත්

```
Coupling එක ඉවත් කරන්නේ  SCHEDULER ට පේන දෙයින්  විතරයි — factory එකෙන් නෙවෙයි.
   → වෙහෙසුණු කෙනා තාමත් වැඩිපුර වැරදි කරනවා
   → scheduler ට ඒක තෝරනකොට පේන්නේ නෑ, එච්චරයි

⛔ Physics එකෙනුත් ඉවත් කළොත් —  හැම ablation එකක්ම වෙනම ලෝකයක ලකුණු කරනවා.
   ඒක B2 එකට couplings අක්‍රීය කරනවා වගේම වැරදියි.
```

### ප්‍රතිඵලය — **design බලාපොරොත්තු වුණාට වඩා පටුයි. ඒක තමයි සොයාගැනීම.**

| CP | ප්‍රතිඵලය |
|---|---|
| **CP5** pace → ergonomics | ★ **මේක විතරයි දත් අල්ලන්නේ.** හැංගුවම shift එකකට **ඇත්ත ergonomic breaches 5.3ක්** — S1 5.43 · S2 4.80 · S3 5.53 (සම්පූර්ණ framework එකේ **හැම scenario එකකම 0**) |
| **CP1** skill → quality | **තීරණයක් වෙනස් වෙන්නේ නෑ** — noise ඇතුළේ |
| **CP2** fatigue → quality | **තීරණයක් වෙනස් වෙන්නේ නෑ** — noise ඇතුළේ |

> Quality weight එක 0.15 → 0.25 කළත් වෙනසක් නෑ ➜ **ඒක tuning ප්‍රශ්නයක් නොවේ.**

**ඇයි? — අනවශ්‍ය බව (redundancy), අදාළ නොවීම නොවේ:**
```
...
```
➜ ඒකයි **CP5 විතරක් බලපාන්නේ.**

**Ablate කරන්න බැරි වුණු 2 — හංගන්නේ නැතුව ප්‍රකාශ කරනවා:**
- **CP3** cognitive load — ගණනය කරනවා, වාර්තා කරනවා, ඒත් **decision layer එක ඒක බලන්නේ නෑ**
- **CP4** task intensity — ඒක fatigue සමීකරණයේ **asymptote** එක, ඇතුළට එන link එකක් නොවේ

> ⚠️ **Test එකෙන් තමන්ගේම දෝෂයක් අල්ලගත්තා:** defect risk නැවත ගණනය කරනකොට
> **shift අවසානයේ** health එක පාවිච්චි කරලා තිබුණා, **task එක දුවපු වෙලාවේ** එක නෙවෙයි.
> ගෙවුණු machine එකක් තනිවම risk එක වැඩි කරනවා ➜ දැන් **සටහන් වුණු තත්ත්වයෙන්** ගණනය කරනවා.

---

## 📦 කොටස 4 — අසමාන workload එක පැහැදිලි කිරීම (`d58e949`)

**ප්‍රශ්නය:** B3 වැඩ **B2 ට වඩා අසමානව** බෙදනවා (S1: Gini **0.084** vs **0.009**).

### පැහැදිලි කිරීම් 2ක් පරීක්ෂා කරලා **ප්‍රතික්ෂේප කළා**

| උත්සාහය | ප්‍රතිඵලය |
|---|---|
| Fairness preference **8 ගුණයක්** වැඩි කළා | 0.0243 → **0.0241**. ග්‍රහණයක් නෑ — **hard constraints තමයි කවුද සුදුසු කියලා තීරණය කරන්නේ**; preference එකට කරන්න පුළුවන් **සුදුස්සන් අතරින් තෝරන එක විතරයි** |
| කාලය වෙනුවට **strain** මැනුවා | scenario 2ක හොඳයි, 1ක නරකයි → **තෝරාගත් එක වාර්තා කරනවා වෙනුවට අත්හැරියා** ✅ |

### ★ ඇත්ත හේතුව — **ශ්‍රම බලකාය**

```
OP1  =  ශක්තිමත්ම operator  (AWL 5.35)
        ඒත් Heavy skill = 0.30  <  competence floor 0.40
        →  ඔහුට heavy වැඩ තහනම්!  ★

→  Heavy වැඩ යන්නේ  ඒක දරාගන්න අඩුම හැකියාව ඇති දෙන්නාට  (OP2, OP3)
```

**ඒ එක skill එක floor එකෙන් උඩට ගත්තොත්:**
```
S1:  Gini  0.0843 → 0.0318   (පරතරයෙන් ~2/3ක් වැහෙනවා)
     Throughput  67.5 → 72.4  ▲
     Breaches    0 → 0        ✅
```
➜ scenario **3ම** පරතරයෙන් ~2/3ක් වැහෙනවා, **3ම** throughput වැඩි වෙනවා.

> ⛔ **මේක රෝග විනිශ්චයක් — ප්‍රතිකාරයක් නොවේ.** Headline ප්‍රතිඵල **ඇත්තටම run කරපු
> configuration එකේම** තියෙනවා. Retrained matrix එක **මේ විශ්ලේෂණය ඇතුළේ විතරයි** —
> අලුත් setup එකක් **මුල් skills ම** ගේනවා කියලා test එකකින් assert කරලා තියෙනවා.

> ⚠️ **පළමු නිගමනය දෙපාරක් වැඩිකර කියලා තිබුණා** — "අඩක්" කිව්වා, ඇත්තටම **2/3ක්**;
> "throughput baseline එකට වඩා වැඩියි" කිව්වා, ඒක **3න් 1ක** විතරයි. **දෙකම නිවැරදි කළා.**

---

## 📦 කොටස 5 — §V Results draft (`c3b8028`)

`paper/section5-results.md` — **පේළි 159, ready-to-paste**. **සංඛ්‍යා 13ක්** result files එකට ගළපලා — **ඔක්කොම ගැලපුණා** ✅

**ලේඛනයේ නීතිය:** සොයාගැනීම් **ධනාත්මකව** කියන්න · trade-off **නිර්මාණ ලක්ෂණ** ලෙස, සමාව අයැදීම් ලෙස නොවේ · **test එකෙන් හම්බුණේ නැති දෙයක් කවදාවත් claim කරන්නේ නෑ**.

> *"අත්හැරපු throughput කියන්නේ, operator කෙනෙක් තමන්ගේ තිරසාර සීමාව පනිනවා නම්
> framework එක **ලබාගන්න ප්‍රතික්ෂේප කරන** throughput එකයි. කිසිවක් අත් නොහරින framework
> එකක් කිසිවක් බලාත්මක කරන්නේ නෑ."*

**File එකේ අගදී "නොකළ යුතු දේ" ලැයිස්තුවක්** තියෙනවා (ඊළඟට edit කරන කෙනාට පේන්න):
```
✗ quality හෝ OEE වැඩි වුණා කියලා claim කරන්න එපා  (p > 0.05)
✗ threshold එක Price ට, rates Calzavara ට ආරෝපණය කරන්න එපා
✗ workload ප්‍රතිඵලය අත්හරින්න එපා
✗ retrained configuration එකේ දෙයක් වාර්තා කරන්න එපා
```

---

## ⚠️ අද අහුවුණු **තමන්ගේම වැරදි 3** — මේ session එකේ ශක්තිය

| # | වැරද්ද | කොහොමද අහුවුණේ |
|---|---|---|
| 1 | KPI % දෙකක් වැරදියි | **Test මුලින්ම ලිව්වා** |
| 2 | Weight sensitivity ගැන **පළමු රෝග විනිශ්චය වැරදියි** — soft penalties (1.70 vs 1.00) චෝදනා කළා. **6 ගුණයකින් අඩු කළා → ප්‍රතිඵල කිසිසේත් වෙනස් වුණේ නෑ** → **මුල් අගය ආපහු දැම්මා** | *"මනින ලද හේතුවක් නැතුව parameter එකක් වෙනස් නොවිය යුතුයි"* |
| 3 | Weight sensitivity tests **දුර්වලයි** — `<=` වලින් දිශාව බැලුවා, **කිසිවක් නොසෙල්වුණත් pass වෙනවා** | දැන් **මනින ලද ප්‍රතිඵලයම** assert කරනවා |
...
| ලිපිගොනුව | කරන්නේ |
|---|---|
| `src/analyse_results.py` + `tests/test_analysis.py` | T7.1, T7.2 · Table II |
| `src/sensitivity_fatigue.py` + tests | T7.6b · λ, μ |
| `src/weight_sensitivity.py` + tests | T7.6 · weights |
| `src/ablation.py` + tests | T7.8 · CP1–CP5 |
| `src/crosstraining.py` + tests | workload diagnosis |
| `results/` | `kpi_table.{csv,md}` · `sensitivity.csv` · `sensitivity_fatigue.csv` · `ablation.csv` · `crosstraining.csv` |
| `paper/section5-results.md` | 🆕 §V draft |

---

# ⛔ පත්‍රිකාවේ ලියන්න **බැරි** දේ (මේ session එකෙන් තහවුරු වුණු)

```
✗ "අපේ weights මගින් trade-off එක පාලනය කරන්න පුළුවන්"
       → මනින්න බැරි තරම් වෙනසයි.  ✅ ලියන්න: "constraints steer, not the objective"
✗ "CP1 සහ CP2 තීරණවලට බලපානවා"
       → බලපාන්නේ නෑ (redundant).  ✅ ලියන්න: CP5 බලපානවා, අනිත් ඒවා redundant paths
✗ "B3 quality හෝ OEE වැඩි කරනවා"
       → p > 0.05.  ✅ ලියන්න: "without degrading quality or OEE"
✗ Workload ප්‍රතිඵලය අත්හැරීම
       → ඒක diagnosis එකක් සමඟ වාර්තා කරන්න — ඒක ශක්තියක්
```
...
│  🔵 PHASE 7 නිම කිරීම                                        │
│                                                              │
│  T7.3  ★ Fig 3 — trade-off / Pareto plot                     │
│  T7.4  Fig 4 — baseline comparison bar chart                 │
│  T7.7  ★ trade-off කතාව  (§V draft එකේ දැනටමත් අඩක් තියෙනවා) │
│                                                              │
│  ඊට පස්සේ →  🔵 PHASE 8 · paper.  §3 මුලින්ම                 │
└──────────────────────────────────────────────────────────────┘
```

## ⚠️ තීරණයක් ඕන — **T5.16 (GATE 3 carry-over) තවම 🟨**

`results/feasibility.csv` එකේ මනිනවා තියෙනවා. Design §12 කියන්නේ **deferrals > 15%** නම් ක්‍රියා කරන්න:

| HC1 | S1 | S2 | S3 |
|---|---|---|---|
| **0.80** (දැන්) | **24.4%** ⚠️ | 11.6% ✅ | **19.5%** ⚠️ |
| 0.85 | **20.0%** ⚠️ | 10.6% ✅ | 14.1% ✅ |

> 0.85 දක්වා ලිහිල් කළත් **S1 තාම 15% ට වඩා වැඩියි**.
> ➜ ★ **Session 10 හි crosstraining ප්‍රතිඵලය මෙතනට උත්තරයක්:** deferrals එන්නේ
> **HC1 නිසා නෙවෙයි — OP1 ට heavy වැඩ තහනම් වීමෙන් (HC2)**. ඒක `config.yaml`
> `constraints.deferral_decision_log` එකේ ලේඛනගත කරලා T5.16 වහන්න පුළුවන්.
